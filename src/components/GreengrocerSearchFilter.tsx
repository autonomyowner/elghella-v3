...
export default function GreengrocerSearchFilter({
  onFilterApply,
  onClose,
  initialSearchQuery = "",
}: FilterProps) {
  ...
  const [GreengrocerType, setGreengrocerType] = useState<string>("");
  ...
  }, [priceRange, rating, GreengrocerType, searchQuery]);

  const GreengrocerTypes = [
    { value: "", label: "اختر نوع المنتج" },
    { value: "الحبوب", label: "الحبوب" },
    { value: "الخضروات", label: "الخضروات" },
    { value: "الفواكه", label: "الفواكه" },
    { value: "الأعلاف", label: "الأعلاف" },
    { value: "المكسرات", label: "المكسرات" },
  ];
  ...
  return (
    <div className=" bg-gray-800 shadow-lg rounded-lg p-6 sticky top-20 border border-gray-700 transition-colors duration-300">
      ...
      {/* Greengrocer Type Filter */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-300 mb-2">
          نوع المنتج
        </label>
        <select
          value={GreengrocerType}
          onChange={(e) => setGreengrocerType(e.target.value)}
          className="w-full border rounded-md p-2 text-sm bg-gray-700 text-gray-100 focus:ring-2 focus:ring-green-600"
        >
          {GreengrocerTypes.map((type) => (
            <option
              key={type.value}
              value={type.value}
